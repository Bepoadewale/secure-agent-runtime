...
`make test` uses a fake backend solely to test control-plane behavior. `make demo` uses the same deterministic test path. The actual Docker backend is used by the API when `POST /run` is called and requires a locally built `agent-runtime-sandbox:local` image; build it with `docker build -t agent-runtime-sandbox:local sandbox/images`. Docker's runtime-default seccomp profile is used; the backend does not override it with a portable-profile name.
