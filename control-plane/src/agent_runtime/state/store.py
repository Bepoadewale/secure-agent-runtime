...
class RuntimeStateStore:
    ...
    def load_tasks(self) -> dict[str, Task]:
        ...
        return {row[0]: Task.model_validate_json(row[1]) for row in rows}
    ...
    def events(self) -> dict[str, list[TaskEvent]]:
        ...
        with self.lock:
            rows = self.connection.execute("SELECT task_id, payload FROM audit_events ORDER BY sequence").fetchall()
    ...
    def load_artifacts(self) -> dict[str, tuple[Artifact, bytes]]:
        with self.lock:
            rows = self.connection.execute("SELECT id, payload, content FROM artifacts").fetchall()
        return {
